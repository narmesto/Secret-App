...
export default function UserRsvpsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      // 1) rsvp rows (order newest-first)
      ...
      if (rErr) {
        console.log("[user rsvps rows error]", rErr.message);
        setEvents([]);
        return;
      }
      ...
      if (eErr) {
        console.log("[user rsvps events error]", eErr.message);
        setEvents([]);
        return;
      }
